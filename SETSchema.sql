...
CREATE OR REPLACE FUNCTION ADD_BOOKING (new_customer VARCHAR, new_performance VARCHAR, new_performance_date DATE, new_booked_by INTEGER, new_instruction VARCHAR, OUT RETURNVALUE INTEGER) AS $$
BEGIN
INSERT INTO BOOKING(CUSTOMER, PERFORMANCE, PERFORMANCE_DATE, BOOKED_BY, INSTRUCTION) VALUES (new_customer, new_performance, new_performance_date, new_booked_by, new_instruction);
END; $$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION UPDATE_BOOKING (new_performance VARCHAR, new_performance_date DATE, new_booked_by INTEGER, new_instruction VARCHAR, select_booking_no INTEGER, OUT RETURNVALUE INTEGER) AS $$
BEGIN
UPDATE BOOKING SET PERFORMANCE = new_performance, PERFORMANCE_DATE = new_performance_date, BOOKED_BY = new_booked_by, INSTRUCTION = new_instruction WHERE BOOKING_NO = select_booking_no;
END; $$ LANGUAGE plpgsql;

CREATE FUNCTION OBTAIN_AGENTID_BY_NAME(IN agentname VARCHAR, OUT result INTEGER) AS $$
DECLARE
userid INTEGER;
BEGIN
SELECT AGENTID INTO userid FROM AGENT WHERE USERNAME = agentname;
result := userid;
END; $$ LANGUAGE plpgsql